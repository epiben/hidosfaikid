---
title: "Development of a prediction model to identify patients at high risk of inappropriate drug dosing in periods with renal dysfunction"
subtitle: "Supplement"
...
for (p in c("tidyr", "ggplot2", "DT", "data.table", "dplyr", "stringr", "readr", "purrr", "forcats", 
			"RPostgreSQL", "umap", "rjson", "patchwork"))
...
dt <- function(d, ...) {
	datatable(d, 
			  ...
			  rownames = FALSE, 
			  options = c(list(pageLength = 12, autoWidth = TRUE, scrollX = TRUE, 
...
study_names <- expand.grid(c("mlp", "linear"), c("not_zero", paste0("geq_", c(1:3, 5)))) %>%
	# slice(1:2) %>% # remove later
	apply(1, function(x) sprintf("binary__%s__daily_rate_%s", x[1], x[2])) %>% 
	setNames(str_replace_all(., c("binary_" = "", 
								  "_mlp" = "MLP", 
								  "_linear" = "Linear",
								  "geq_" = ">= ",
								  "not_zero" ="> 0",
...
```{r table_S1}
read_tsv("09-manually_entered/09-table_s1__egfr_thresholds__max_daily_doses.tsv", trim_ws = FALSE) %>% 
	datatable(rownames = FALSE, 
			  options = c(list(pageLength = 20, autoWidth = FALSE, scrollX = FALSE, 
			  				 searching = TRUE, lengthChange = FALSE, 
			  				 dom = "t"))) %>% 
	formatStyle(1, whiteSpace = "pre") # retain white-space indenting
```
...
read_tsv("05-table_1_summary_stats.tsv", trim_ws = FALSE) %>% 
	...
	datatable(filter = list(position = "top", clear = TRUE), 
		  rownames = FALSE, 
		  options = c(list(pageLength = 20, autoWidth = FALSE, scrollX = FALSE, 
		  				 lengthMenu = list(c(10, 20, 40, 60, -1), c(10, 20, 40, 60, "All")),
		  				 searching = TRUE, lengthChange = TRUE, 
		  				 search = list(regex = TRUE)))) %>% 
	formatStyle(1, whiteSpace = "pre") %>% # retain 3-whitespace indenting
	formatStyle(2:4, textAlign = "center")
```
...
```{r umap, fig.height = 7, fig.width = 10, eval=FALSE, include=FALSE}
##### Figure S2: Visualisation of 2-dimensional Uniform Manifold Approximation and Projection (UMAP) analysis 5,000 samples samples from the full data set. Points are coloured according to each of the five outcome variables modelled. The figure illustrates the lack of clear boundaries between the labels (outcome variables' values), suggesting the prediction be difficult. 

set.seed(42)
umap_data <- dbGetQuery(conn, "SELECT * FROM hidosfaikid.data_dev") %>% 
	select(-admission_id, -person_id, -r, -time_at_risk) %>% 
	sample_n(5000)

umap_object <- select(umap_data, -starts_with("daily_rate_")) %>% 
	as.matrix() %>% 
	umap(random_state = 42)

bind_cols(select(umap_data, starts_with("daily_rate_")), data.frame(umap_object$layout)) %>% 
	pivot_longer(-c("X1", "X2"), names_to = "outcome_variable", values_to = "outcome_value") %>% 
	mutate_at("outcome_variable", factor,
			  levels = paste0("daily_rate_", c("not_zero", paste0("geq_", c(1:3, 5)))),
			  labels = sprintf("Daily rate %s", c(">0", paste0(">=", c(1:3, 5))))) %>% 
	mutate_at("outcome_value", factor, levels = 0:1, labels = c("No", "Yes")) %>% 
	ggplot(aes(X1, X2, colour = outcome_value)) +
		geom_jitter(size = 0.5, alpha = 0.7) +
		labs(x = "1st UMAP dimension", y = "2nd UMAP dimension", colour = "Outcome?") +
		scale_colour_brewer(palette = "Set1") +
		facet_wrap(~ outcome_variable, ncol = 3) +
		coord_equal() +
		theme_minimal() +
...
```{r optuna_learning, fig.height = 8, fig.width = 10, results = "asis", echo = FALSE}
...
```{r roc_curves, fig.height = 4, fig.width = 10, results = "asis", echo = FALSE}
for (sn in names(study_names)) {
	fig_counter <- fig_counter + 1
	cat(sprintf("\n##### Figure S%s: %s.\n", fig_counter, study_names[sn]))
...
```{r calibration_curves, fig.height = 4, fig.width = 10, results = "asis", echo = FALSE}
for (sn in names(study_names)) {
	fig_counter <- fig_counter + 1
	cat(sprintf("\n##### Figure S%s: %s.\n", fig_counter, study_names[sn]))
...
```{r decision_curves, fig.height = 4, fig.width = 10, results = "asis", echo = FALSE, warning = FALSE}
for (sn in names(study_names)) {
	fig_counter <- fig_counter + 1
	cat(sprintf("\n##### Figure S%s: %s.\n", fig_counter, study_names[sn]))
